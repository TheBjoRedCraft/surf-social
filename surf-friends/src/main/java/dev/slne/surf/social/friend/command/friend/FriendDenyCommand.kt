...
class FriendDenyCommand(commandName: String) : CommandAPICommand(commandName) {
    init {
        withPermission("surf.social.friend.command.deny")
        ...
        executesPlayer(PlayerCommandExecutor { player: Player, args: CommandArguments ->
            pluginContainer.launch {
                ...
                user.denyFriendRequest(targetUser)
                SurfFriends.send(player, MessageBuilder().primary("Du hast die Freundschaftsanfrage von ").info(target.name).error(" abgelehnt."))
            }
        })
    }
}
