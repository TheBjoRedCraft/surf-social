...
class ChannelInviteRevokeCommand(commandName: String) : CommandAPICommand(commandName) {
    init {
        ...
        executesPlayer(PlayerCommandExecutor { player: Player, args: CommandArguments ->
            ...
            SurfChat.Companion.send(
                player, MessageBuilder().primary("Du hast die Einladung für ").info(
                    target!!.name!!
                ).primary(" in den Nachrichtenkanal ").info(channel.name).success(" zurückgezogen.")
            )
            SurfChat.Companion.send(
                target,
                MessageBuilder().primary("Deine Einladung in den Nachrichtenkanal ")
                    .info(channel.name).success(" wurde zurückgezogen.")
            )
        })
    }
}
